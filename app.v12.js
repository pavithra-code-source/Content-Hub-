document.addEventListener('DOMContentLoaded', () => {
    ...
    // A. Wire Sidebar Links & View Switching
    const sidebarLinks = document.querySelectorAll('.nav-links li, .sidebar-bottom li');
    ...
    sidebarLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            const viewId = viewMap[sectionName];
            
            if(!viewId) {
                 showToast(`Feature Demo: ${sectionName} is coming soon!`, 'lock');
                 return;
            }
            ...
            if(sectionName === 'Inbox') {
                headerTitle.innerHTML = 'Unified Inbox <span class="badge">AI Assisted</span>';
            } else {
            ...
            showToast(`Loading ${sectionName}...`, 'layout');
        });
    });
    ...
    // ---- 6. Real-Time Phone Call Simulation Logic ----
    ...
    function startCallSimulation() {
        ...
        speakText("Incoming call from +91 91 2 34 56 7 8 9. Incoming call.");
        
        // Auto-miss after 10 seconds if no action
    ...
    function handleMissedCall() {
        ...
        // Use custom auto-reply from settings if available
        const autoReplyEl = document.getElementById('setting-auto-reply');
        const customMsg = autoReplyEl ? autoReplyEl.value : "We missed your call, we will get back to you!";
        
        showToast("Call Missed. AI follow-up: " + customMsg.substring(0, 30) + "...", "phone-missed");
        
        // Inject into Missed Calls View
        const missedCallsTable = document.querySelector('#view-missed-calls tbody');
        if(missedCallsTable) {
            const newRow = `
                <tr>
                    <td>+91 91234 56789</td>
                    <td>0:00</td>
                    <td>Auto-Reply: "${customMsg.substring(0, 40)}..."</td>
                    <td><span class="status-pill called">Handled</span></td>
                </tr>
            `;
            missedCallsTable.insertAdjacentHTML('afterbegin', newRow);
        }

        speakText(`Call missed. I have automatically sent your custom WhatsApp reply: ${customMsg}`);
    }

    if(simulateCallBtn) {
        simulateCallBtn.addEventListener('click', () => {
            showToast("Simulating connection to your physical phone...", "smartphone");
            setTimeout(startCallSimulation, 1500);
        });
    }
    ...
    if(answerCallBtn) {
        answerCallBtn.addEventListener('click', () => {
            clearTimeout(callTimeout);
            callModal.classList.remove('active');
            showToast("Connecting to VoIP line...", "phone-call");
            speakText("Connecting the call. Please start speaking.");
        });
    }

    if(rejectCallBtn) {
        rejectCallBtn.addEventListener('click', handleMissedCall);
    }

    // ---- 7. USB Bridge Polling Logic ----
    const usbStatusBadge = document.getElementById('usbStatusBadge');
    let bridgeLastEventTime = 0;
    let bridgeActive = false;

    async function pollPhoneBridge() {
        try {
            ...
            if (!bridgeActive) {
                bridgeActive = true;
                usbStatusBadge.style.color = 'var(--success)';
                usbStatusBadge.style.background = 'rgba(16, 185, 129, 0.1)';
                usbStatusBadge.style.borderColor = 'rgba(16, 185, 129, 0.2)';
                usbStatusBadge.innerHTML = '<i data-feather="usb"></i> USB: Online';
                feather.replace();
                showToast("Physical Phone Bridge Connected!", "link");
            }

            // Detect new events (ignore old timestamps)
            if (data.timestamp > bridgeLastEventTime) {
                if (data.status === "Ringing" && !callModal.classList.contains('active')) {
                    startCallSimulation();
                    // Update number if bridge provides one
                    if (data.number !== "Unknown") {
                        document.querySelector('#callModal h2').innerText = data.number;
                    }
                } else if (data.status === "Missed" && callModal.classList.contains('active')) {
                    handleMissedCall();
                }
                bridgeLastEventTime = data.timestamp;
            }

        } catch (err) {
            if (bridgeActive) {
                bridgeActive = false;
                usbStatusBadge.style.color = 'var(--text-secondary)';
                usbStatusBadge.style.background = 'rgba(255,255,255,0.05)';
                usbStatusBadge.style.borderColor = 'var(--border-color)';
                usbStatusBadge.innerHTML = '<i data-feather="usb"></i> USB: Offline';
                feather.replace();
            }
        }
    }
    ...
    setInterval(pollPhoneBridge, 2000);
    ...
    const settingAiVoice = document.getElementById('setting-ai-voice');
    const settingAutoReply = document.getElementById('setting-auto-reply');
    ...
    function closePModal() { profileModal.classList.remove('active'); }
    if(closeProfileModal) closeProfileModal.addEventListener('click', closePModal);
    if(cancelProfileBtn) cancelProfileBtn.addEventListener('click', closePModal);

    // Save Profile
    if(saveProfileBtn) {
        saveProfileBtn.addEventListener('click', () => {
            const newName = editOwnerName.value.trim();
            if(newName) {
                sidebarUserName.innerText = newName;
                settingBizName.value = newName;
    ...
    if(saveSettingsBtn) {
        saveSettingsBtn.addEventListener('click', () => {
            const newBizName = settingBizName.value.trim();
            const voiceGen = settingAiVoice.value;
            const newAutoReply = settingAutoReply.value;
            
            sidebarUserName.innerText = newBizName;
            editOwnerName.value = newBizName;
            
            showToast("Settings & Auto-Reply Saved!", "save");
            speakText(`Settings updated. AI Persona voice is now set to ${voiceGen}. Business name is ${newBizName}. Your auto-reply message has been updated as well.`);
        });
    }

    // ---- 9. New Appointment Logic ----
    ...
    function openApptModal() { appointmentModal.classList.add('active'); }
    function closeApptModalFn() { appointmentModal.classList.remove('active'); }

    if(newAppointmentBtn) newAppointmentBtn.addEventListener('click', openApptModal);
    if(closeApptModal) closeApptModal.addEventListener('click', closeApptModalFn);
    if(cancelApptBtn) cancelApptBtn.addEventListener('click', closeApptModalFn);

    if(saveApptBtn) {
        saveApptBtn.addEventListener('click', () => {
            const name = document.getElementById('appt-cust-name').value || "New Customer";
            const service = document.getElementById('appt-service').value;
            const timeRaw = document.getElementById('appt-time').value;
            
            // Format time for card
            const hour = parseInt(timeRaw.split(':')[0]);
            const min = timeRaw.split(':')[1];
            const ampm = hour >= 12 ? 'PM' : 'AM';
            const displayHour = hour % 12 || 12;
            const displayTime = `${displayHour}:${min}`;

            const apptHtml = `
                <div class="appointment-card" style="background: var(--panel-bg); border: 1px solid var(--border-color); padding: 16px; border-radius: 12px; display: flex; align-items: center; gap: 20px; animation: modalIn 0.3s ease;">
                    <div class="time-box" style="width: 80px; text-align: center; border-right: 1px solid var(--border-color); padding-right: 20px;">
                        <div style="font-size: 18px; font-weight: 700;">${displayTime}</div>
                        <div style="font-size: 11px; color: var(--text-secondary);">${ampm}</div>
                    </div>
                    <div class="appointment-info" style="flex: 1;">
                        <h4 style="font-size: 15px; margin-bottom: 4px;">${name}</h4>
                        <p style="font-size: 13px; color: var(--text-secondary);"><i data-feather="tool" style="width: 12px;"></i> ${service}</p>
                    </div>
                    <div class="appointment-status">
                        <span class="status-pill warm">Booked</span>
                    </div>
                    <button class="btn btn-outline btn-sm">Check-in</button>
                </div>
            `;

            appointmentsGrid.insertAdjacentHTML('afterbegin', apptHtml);
            feather.replace();
            
            showToast("Appointment successfully scheduled!", "calendar");
            speakText(`Confirmed. Scheduling ${service} for ${name} at ${displayTime} ${ampm}.`);
            
            closeApptModalFn();
        });
    }

    // Mock Profile Pic Click
    const avatarUpload = document.querySelector('#profileModal .avatar.lg');
    if(avatarUpload) {
        avatarUpload.addEventListener('click', () => {
            showToast("Select Image feature restricted in demo.", "image");
        });
    }
